...
class ProjectDetail extends React.Component {
  ...
  async contributeProject() {
    ...
    // 字段合规检查
    if (amount <= 0) {
      return this.setState({ errmsg: 'Bet amount must be larger or equal than 0' });
    }
    ...
    try {
      ...
      this.setState({ errmsg: 'Bet Success!', amount: 0 });
      console.log(result);
  ...
  async approvePayment(i) {
    try {
      ...
      if (!isInvestor) {
        return window.alert('Only Betters can vote!');
      }
  ...
  async doPayment(i) {
    try {
      ...
      if (sender !== this.props.project.owner) {
        return window.alert('This can only be done by betters!');
      }
  ...
  render() {
    ...
    return (
      <Layout>
        ...
        <Typography variant="determinate" color="inherit" style={{ margin: '30px 0 15px' }}>
          Draw will automatically start after the bet limit reached. You can also request to draw before then.
        </Typography>
    ...
    return (
      <Paper style={{ padding: '15px' }}>
        ...
        <Grid container spacing={16}>
          ...
          <InfoBlock title={`${project.investorCount}`} description="Participants" />
          <InfoBlock title={`${web3.utils.fromWei(project.balance, 'ether')} ETH`} description="Current Prize Pool" />
        </Grid>
        <Grid container spacing={16}>
          <Grid item md={12}>
            <TextField
            ...
            <Button size="small" variant="raised" color="primary" onClick={this.onSubmit}>
              {this.state.loading ? <CircularProgress color="secondary" size={24} /> : 'Bet Now!'}
            </Button>
            {!!this.state.errmsg && (
        ...
        <Table style={{ marginBottom: '30px' }}>
          <TableHead>
            <TableRow>
              ...
              <TableCell numeric>Lottery Amount</TableCell>
  ...
  renderPaymentRow(payment, index, project) {
    ...
    return (
      <TableRow key={index}>
        ...
        <TableCell>{payment.completed ? 'Si' : 'No'}</TableCell>
        <TableCell>
